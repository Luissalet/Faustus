...
| Correo | `/email` | `/email` («Herramientas», paleta, `/email`): cuenta (selector si hay varias), carpetas, filtros (todos, sin leer, sin contestar, con estrella), lista paginada con no leídos en negrita, estrella y clip, búsqueda en la carpeta, lectura con HTML en marco aislado y adjuntos descargables, responder / responder a todos / reenviar con cita, archivar, borrar, mover a carpeta, estrella, marcar como no leído; redactar con CC/CCO, enviar (`/send`) y guardar borrador (`/draft`); actualizar | **Parcial** — cuentas (IMAP/SMTP, Google OAuth, prueba), IA (resumir, traducir, respuesta con IA, estilo), programar envíos y aprobaciones pendientes, adjuntar ficheros al redactar, unsubscribe, reglas y etiquetas, contactos, reminders desde el correo, estado de urgencia, imágenes en línea: anterior |
