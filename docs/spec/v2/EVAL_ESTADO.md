...
| EVAL-04 | verde | `scripts/ui_smoke.py` | Real, con Chromium, contra el build servido (`static/studio/`): los 6 pasos en verde (login, nueva conversación, envío de mensaje + tarjeta `ask_user`, y los tres paneles de Settings). El bug de `owner=None` **se corrigió en el lote 36**; el hallazgo NUEVO del paso 3 (routing "low signal") **se corrigió en el lote 40**, ver abajo. Re-confirmado en el cierre de lotes 60-70 (Lote 60): los 3 pasos del guion siguen en verde tras la ronda de cableado de ficheros ajenos, sin regresión nueva. |
