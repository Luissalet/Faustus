...
Resumen (26-09-2026): **47 verde**, **0 xfail**, **1 manual** — 48 de 48. QA-44 pasó a verde: el recorrido de teclado y zoom da 38/38 en cuatro corridas seguidas (ver su fila). Antes (cierre lotes 60-70, 2026-09-11): 46 verde, 1 xfail, 1 manual. QA-10 y QA-27 pasaron a verde en el Lote 64/66 (`resume_interrupted`/`remote_worker_registry` reconciliation, ver sus filas). QA-44 sigue xfail: los huecos 1 y 2 (dialog control/diff region @ 200%) se corrigieron en Studio en el Lote 65; el hueco 3 (Escape intermitente) requiere un entorno de ejecución estable, no solo más código — ver su fila.
...
| QA-44 | verde | test_qa_44_teclado_y_zoom.py | 26-09-2026: `scripts/ui_a11y.py` da 38/38 en cuatro corridas seguidas (chat, tarjeta de aprobación, diff y diálogo, sólo teclado, a 100 % y 200 %). Los fallos que quedaban eran de medida del script: el 200 % se emulaba con `zoom` de CSS (ahora una ventana de 640x430 px CSS, lo que la página ve a ese zoom), el anillo de foco se leía a mitad de su transición y la posición antes de que terminara el desplazamiento suave. El Escape anota el foco y admite que el primer Escape cierre algo dentro del diálogo. El guion pide tarjeta con una regla de argumento para `edit_file` y aprueba con «Allow for this task». |
