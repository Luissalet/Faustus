...
| `turn_error` | `routes/chat_routes.py`'s two `/api/chat_stream` terminal-failure branches (the plain-chat `event: error` terminal and the agent-mode `agent_terminal` failure), right where each sets the stream status to `"error"` — once per failed turn, never for a user/client cancellation (`CancelledError`/`GeneratorExit`, which save the partial reply directly and never reach this branch) | the session's name | the terminal failure text saved with the turn (e.g. `"…\n\n[Response stopped: Model request failed (HTTP 503)]"`) — never the raw provider error detail |
