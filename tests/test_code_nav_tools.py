...
@pytest.fixture
def repo():
    # Built under the system temp dir, which is on the default tool-path
    # allowlist (/tmp on POSIX, %TEMP% on Windows).
...
def test_grep_finds_match(repo):
    r = _run("grep", f'{{"pattern": "needle", "path": {json.dumps(repo)}}}')
...
def test_grep_ignore_case(repo):
    r = _run("grep", f'{{"pattern": "needle", "ignore_case": true, "path": {json.dumps(repo)}}}')
    assert "b.txt:2:" in r["output"]


def test_grep_glob_filter(repo):
    r = _run("grep", f'{{"pattern": "needle", "ignore_case": true, "glob": "*.py", "path": {json.dumps(repo)}}}')
    assert "a.py" in r["output"]
    assert "b.txt" not in r["output"]


def test_grep_no_match(repo):
    r = _run("grep", f'{{"pattern": "zzzznotfound", "path": {json.dumps(repo)}}}')
    assert r["exit_code"] == 0
...
def test_grep_skips_case_variant_sensitive_files_rg(repo):
    ...
    r = _run("grep", f'{{"pattern": "{token}", "path": {json.dumps(repo)}}}')
    assert r["exit_code"] == 0
...
def test_glob_py(repo):
    r = _run("glob", f'{{"pattern": "*.py", "path": {json.dumps(repo)}}}')
    assert r["exit_code"] == 0
    assert "a.py" in r["output"]
...
def test_glob_literal_in_subdir(repo):
    ...
    r = _run("glob", f'{{"pattern": "b.txt", "path": {json.dumps(repo)}}}')
    assert r["exit_code"] == 0
...
def test_glob_multi_segment_single_star(repo):
    ...
    r = _run("glob", f'{{"pattern": "sub/*.txt", "path": {json.dumps(repo)}}}')
    assert r["exit_code"] == 0
    assert "b.txt" in r["output"]
...
def test_glob_star_does_not_cross_slash(repo):
    ...
    r = _run("glob", f'{{"pattern": "sub/*.py", "path": {json.dumps(repo)}}}')
    assert r["exit_code"] == 0
...
def test_ls_lists_entries(repo):
    r = _run("ls", f'{{"path": {json.dumps(repo)}}}')
    assert r["exit_code"] == 0
    assert "a.py" in r["output"]
...
def test_read_file_offset_limit(repo):
    ...
    r = _run("read_file", f'{{"path": {json.dumps(p)}, "offset": 3, "limit": 2}}')
    assert r["exit_code"] == 0
