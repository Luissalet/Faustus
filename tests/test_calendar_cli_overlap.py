"""Regression: `faustus-calendar list` must select events that OVERLAP the
...
`faustus-calendar list` with the default start=now()).
"""
...
def _load_cli(monkeypatch, rows):
    ...
    path = ROOT / "scripts" / "faustus-calendar"
    loader = importlib.machinery.SourceFileLoader("faustus_calendar_cli", str(path))
