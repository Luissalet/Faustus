...
defused links, and the rule that unknown syntax falls through as text —
and prints ok/FAIL lines; this test runs it. Needs node and the repo's
node_modules (esbuild bundles the TS).
"""
import shutil
import subprocess
...
_CHECK = _REPO / "studio" / "checks" / "markdown.check.mjs"
...
def test_studio_markdown_checks_pass():
    proc = subprocess.run(
