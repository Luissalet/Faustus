...
class TestRunningInContainer:
    """Detect whether the Faustus process itself runs inside a container."""
...
class TestPackageProbeStatus:
    ...
    def test_vllm_cli_without_dist_is_external_for_update(self):
        ...
        assert "outside Faustus" in status.note
    ...
    def test_apfel_does_not_use_generic_outside_odysseus_note(self):
        ...
        assert status.available is False
        assert "Update this system dependency outside Faustus." not in status.note
    ...
    @pytest.mark.skipif(os.name == "nt", reason="POSIX-only: ~/.local/bin user-base layout")
