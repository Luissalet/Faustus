...
@pytest.mark.parametrize(
    ("value", "expected"),
    [
        ("http://faustus", ("faustus", 80)),
        ("http://faustus:7000", ("faustus", 7000)),
        ("http://localhost:7000", ("localhost", 7000)),
        ("http://faustus.local", ("faustus.local", 80)),
        ("http://api.faustus.local:7000", ("api.faustus.local", 7000)),
...
@pytest.mark.parametrize(
    ...
    [
        ...
        "ftp://faustus.example",
        "https://faustus.local",
        "http://user:password@faustus.local",
        "http://faustus.local/",
        "http://faustus.local/path",
        "http://faustus.local?query=1",
        "http://faustus.local#fragment",
        "http://faustus.local:not-a-port",
        "http://faustus.local:0",
        "http://faustus.local:65536",
        "http://faustus.local:07000",
        ...
        "http://faustus%2elocal:7000",
        "http://%31%39%32.168.1.9:7000",
        "http://faustus%40local:7000",
        "http://.local:7000",
        "http://faustus..local:7000",
        "http://faustus.local.:7000",
        "http://-faustus:7000",
        "http://faustus-:7000",
        "http://faustus_name:7000",
        f"http://{'a' * 64}:7000",
...
def test_pair_post_json_prefers_configured_origin(monkeypatch):
    monkeypatch.setenv("COMPANION_BASE_URL", "http://faustus.local:7000")
    mint = MagicMock(return_value=("tok123", "ody_raw"))
    ...
    assert response["host"] == "faustus.local"
    assert response["port"] == 7000
    assert response["hosts"] == ["faustus.local"]
    assert set(response) == {
        "host",
        "port",
        "token",
...
def test_pair_post_rejects_invalid_config_before_mint_without_echoing_it(monkeypatch):
    ...
    monkeypatch.setenv(
        "COMPANION_BASE_URL",
        f"http://admin:{configured_secret}@faustus.local",
    )
