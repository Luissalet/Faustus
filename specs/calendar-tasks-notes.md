...
- direct DB CLIs `scripts/faustus-calendar`, `scripts/faustus-notes`, and `scripts/faustus-tasks`;
...
- route-level no-login calendar access normalizes empty owner values to `FAUSTUS_FALLBACK_OWNER` or `owner@localhost`, so route-created calendar rows do not use the empty string as their storage owner;
...
- the in-process scheduler is gated by `FAUSTUS_INPROCESS_TASKS`, and multiple enabled app processes can double-run work.
...
- `scripts/faustus-webhook` builds the live `/api/tasks/{task_id}/webhook/{token}` path with percent-encoded path segments; its direct DB token rotation/revocation behavior remains a local compatibility surface.
