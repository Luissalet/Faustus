...
When a request clearly belongs to a domain (calendar, notes, tasks, email, WhatsApp, contacts, documents, chats, desktop, media, integrations), that domain's everyday tools go out as full schemas on the first round even when vector retrieval missed them, so the model does not spend a round on `lookup_tools` first. The tool index embeds only the MCP tools whose description changed; a server connecting or disconnecting no longer re-embeds all of them.
