...
- **Creator** (flag `creator_enabled`, off by default) — a media production domain with its own document model, library and lineage, model-identity-vs-deployment separation, capability/param contracts, preflight and per-project budget, physical-resource admission, bounded ingest, a `/creator` Studio shell and a safe plugin lifecycle.
