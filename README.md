...
- **Your WhatsApp, read and answered from here** — pair your own account by QR (WhatsApp Web multi-device protocol, a local Node bridge on the loopback with a token), then ask "what did people write me today", "summarise what Ana said", or "tell Ana I'll be ten minutes late" — every send is approved by you; a daily WhatsApp digest can be a Home card; Tools → WhatsApp shows chats, messages and a composer.
...
- **Code graph** answers architecture and call-tracing questions — trace a path between two symbols, see what a diff's callers are, get a repo's route/hotspot map — without reading whole files.
