...
- **Bundled libraries**: 33 installable skills (TDD, verification loop, context budget, security review, onboarding, API design, evals, web accessibility/UI design, an interactive "learn this repo" tutor that studies the code graph's modules and flows with you…), 52 per-language rule files (`common` plus Python, TypeScript, React, Go, Rust, Java, C#, Swift, Kotlin, PHP, web) that join the system prompt behind the project's own `.faustus/rules/` under a token budget and the same trust verdict as the instructions, and 19 agent definitions (language reviewers that cannot write, build-error resolver, TDD guide, architect…). Skills → Library, Project → Rules. [skill_library.py](src/skill_library.py) · [project_rules.py](src/project_rules.py).
