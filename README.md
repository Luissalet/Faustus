...
- **Side threads:** select a passage in a reply and *Explore separately*. A new thread opens that sees the conversation up to that turn plus the passage, and grows on its own; the original conversation does not change. Bring the result back as an explicit reference block (latest exchange, or the whole thread), change its depth, withdraw it or remove it — what the model sees next is exactly what is wired, and the *Side threads* panel shows it layer by layer with token counts, a thought map of the thread tree, and a stale mark when a referenced thread has moved on. Inspired by [ThoughtDAG](https://github.com/chenxiachan/thoughtdag)'s one rule: wires are the context.
- **Context wires, replay and condense:** pin a document selection or a note so it stays in context until you withdraw it; every reply records which wires it was written against, so when a side thread or document moves on, the affected replies say so and offer *Regenerate with the current version* — nothing regenerates on its own. Pick any range of settled turns and *Condense up to here*: one summary row replaces them (same summarizer as automatic compaction, on your own model), with *Expand* restoring the originals byte for byte.
