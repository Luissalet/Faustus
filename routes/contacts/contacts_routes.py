...
def _save_local_contacts(contacts: List[Dict]) -> None:
    ...
    _contact_cache["fetched_at"] = datetime.now(_utc_timezone.utc).replace(tzinfo=None)
...
def _fetch_contacts(force=False):
    ...
    if not force and _contact_cache["fetched_at"]:
        age = (datetime.now(_utc_timezone.utc).replace(tzinfo=None) - _contact_cache["fetched_at"]).total_seconds()
        if age < 60:
...
def setup_contacts_routes():
    ...
    async def export_contacts(
        ...
        if format == "csv":
            ...
            filename = "faustus-contacts.csv"
        else:
            content = _contacts_to_vcf(contacts)
            media_type = "text/vcard; charset=utf-8"
            filename = "faustus-contacts.vcf"
        return Response(
            content=content,
