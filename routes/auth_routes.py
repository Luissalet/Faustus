...
def setup_auth_routes(auth_manager: AuthManager) -> APIRouter:
    ...
    async def set_settings(request: Request):
        ...
        _INT_RANGES = {
            ...
            # Job D: token budgets for the skill index (level 0) and the
            # procedure body (level 1) — src/skills_runtime/disclosure.py.
            "skill_list_budget_tokens": (50, 4000),
            "skill_body_budget_tokens": (100, 16000),
    ...
    async def test_integration_route(integration_id: str, request: Request):
        ...
        if preset == "ntfy":
            ...
            headers = {
                "Title": "Faustus connectivity test",
            ...
            try:
                async with httpx.AsyncClient(timeout=8.0) as client:
                    r = await client.post(
                        full_url,
                        content="Connectivity test from Faustus. If you see this on your phone, ntfy is wired up correctly.",
                        headers=headers,
                    )
                if r.is_success:
