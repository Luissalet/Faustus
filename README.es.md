...
- **Creator** (ajuste `creator_enabled`, apagado por defecto) — un dominio de producción multimedia con su propio modelo de documento, biblioteca y linaje, separación identidad de modelo vs. despliegue, contratos de capacidad/parámetro, preflight y presupuesto por proyecto, admisión de recursos físicos, ingesta acotada, un shell `/creator` en el Studio y un ciclo de vida seguro de plugins.
