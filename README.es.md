...
- **Paridad con TrueForge medida, no proclamada**: la matriz de paridad y las 36 recetas de aceptación de una auditoría independiente viven en `docs/spec/trueforge/`; `scripts/acceptance_run.py` ejecuta las que tienen test real y escribe una fila de evidencia por caso (commit, hash de configuración, modelo, resultado, estado del coste). Hoy hay siete en verde: admisión de turnos que nunca cancela un run válido por una petición inválida, aprobaciones con una sola decisión y recibo consistente, replay por cursor sin repetir al modelo, seguimiento de efectos desconocidos en llamadas interrumpidas, fencing de leases con generaciones y cancelación que llega a nietos, aprobaciones y preguntas ([ejecutor de aceptación](docs/api/acceptance_runner.md)).
