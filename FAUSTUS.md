...
- **Servidor MCP `mcp_servers/workers_server.py`** para Claude Desktop / Cowork / Claude Code: `workers_guide` (cómo usar bien a los workers: qué delegar, cómo escribir una tarea, cómo leer el resultado, el bucle plan → dispatch → wait → check), `dispatch_workers`, `workers_wait`, `workers_status`, `workers_events`, `workers_cancel`, `workers_list`. Y una **skill** lista (`integrations/faustus-workers/SKILL.md`) para que cualquier modelo tipo Fable delegue solo. Documentado en `website/fable-workers.md`.
