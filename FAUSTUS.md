...
**Seguridad y límites.** El puente solo escucha en 127.0.0.1 con token; enviar exige una persona (ruta humana o tarjeta de permiso del agente); los textos ajenos entran como datos. Protocolo no oficial: WhatsApp puede pedir volver a emparejar; no se usa para envíos masivos. Sin multimedia (se ven como `[image]`, `[audio]`…), sin grupos nuevos, sin reacciones. El emparejamiento lo hace Luis con el móvil — no se puede automatizar.

**Ficheros.** `bridges/whatsapp/{server.mjs,package.json}`, `src/whatsapp_bridge.py`, `src/whatsapp_tools.py`, `routes/whatsapp_routes.py`, registro en `tool_schemas`/`agent_tools`/`tool_execution`/`agent_loop`/`tool_index*`/`tool_capabilities`/`tool_policy`, `src/watchers.py` (`whatsapp_digest`), `studio/src/adapters/whatsapp.ts`, `studio/src/screens/whatsapp/*`, `studio/checks/whatsapp.check.mjs`, `tests/test_whatsapp_bridge.py` (8), `tests/test_whatsapp_bridge_js.py`, `tests/test_studio_whatsapp_js.py`.
