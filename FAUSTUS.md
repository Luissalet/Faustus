...
- Cifras a 03-09-2026 (00:00, en `master`): **216 commits**, +71.500 líneas sobre la base (498 ficheros tocados); **56 módulos nuevos** en `src/`, `routes/` y `static/js/`, **115 ficheros de tests nuevos** (940 en total). Suite completa: **7.733 tests en verde**, 2 saltados, ~6 min en Linux (1 fallo preexistente del entorno: `markitdown` sin conversor docx); e2e Playwright 12 flujos.
...
- **Servidor MCP `mcp_servers/workers_server.py`** para Claude Desktop / Cowork / Claude Code: `workers_guide` (cómo usar bien a los workers: qué delegar, cómo escribir una tarea, cómo leer el resultado, el bucle plan → dispatch → wait → check), `dispatch_workers`, `workers_wait`, `workers_status`, `workers_events`, `workers_cancel`, `workers_list`. Y una **skill** lista (`docs/skills/faustus-workers/SKILL.md`) para que cualquier modelo tipo Fable delegue solo. Documentado en `docs/FABLE_WORKERS.md`.
