...
**Verificado.** `tests/test_gpu_topology.py`, `test_memory_budget.py`, `test_hardware_routes.py`, `test_vram_admission_reconciled.py` (20), `test_inf05_serve_gate.py` (8), `test_bench_profiles_activation.py` (13), `test_inf05_hardware_js.py`, contratos/recibos/arquitectura ampliados; los tests históricos de admisión (`test_vram_admission`, `test_hw_vram_reservation`, `test_l66_hw01_concurrent_admit`, QA-24, `test_chat_vram_gate`) siguen verdes. Todo con fixtures: **ningún `nvidia-smi` real ni carga real en tests**. Lo que sólo el hardware puede validar: la descarga automática bajo presión real, la reconexión de una eGPU con cambio de índices, y el ajuste `fitted` (hoy `KV_RATES` guarda una observación por modelo, así que `fitted` casi nunca se dispara hasta que acumule varias).
