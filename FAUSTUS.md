...
## 94. Plan Creator: WP00-WP12/27/30/32 (16-09-2026)
...
**Estado honesto.** `creator_enabled` sigue en OFF por defecto — nada de esto es visible a un usuario que no lo active a mano. WP02/WP09/WP30 están implementados y probados pero NO montados en `app.py` (fuera de la propiedad de ficheros de sus lotes; el diff exacto vive en `WP0{2,9}_wiring.md`/`WP30_wiring.md`). El shell del Studio (`WP05`, `/creator`) es solo capa de presentación sobre la API que ya existía — sin backend nuevo. Quedan 30+ paquetes del plan de 43 sin empezar (WP08, WP11, WP13-16, WP20, WP24, WP36 entre los siguientes candidatos) — ver OBJ-14.

**Ficheros.** `src/creator/` (`documents,library,ingest,capabilities,preflight,adapter_port,adapters/,ops/,goal,resources,plugins`.py), `src/model_identity.py`, `routes/creator_*.py`, `studio/src/screens/creator/CreatorScreen.tsx`, `src/vram_admission.py`, `src/run_model_pin.py`, `docs/spec/creator/{00_WP00_INVENTARIO,WP01,WP02,WP03,WP04,WP05,WP06,WP07,WP09,WP10,WP12,WP27,WP30,WP32}.md`.
