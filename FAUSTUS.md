...
- Rama: **una sola, `master`** (`D:\LocalAI\faustus`), que trackea `origin/master` en `github.com/Luissalet/Faustus`. Las ramas `feat/projects` y `feat/reliability` y la worktree de pruebas se consolidaron el 31-08.
...
**Actualización (18-09-2026):** esa decisión se revierte. Se hace un renombrado completo y limpio de todo identificador interno que quedaba en `odysseus`/`ODYSSEUS_*`: variables de entorno, la cookie de sesión, las claves de `localStorage`, la carpeta de convención de proyecto (`.odysseus/` → `.faustus/`), scripts de `scripts/`, la unidad de systemd, el `.spec` de PyInstaller, cabeceras HTTP internas y demás. La única compatibilidad que queda, y solo de lectura, es: (1) una carpeta de proyecto `.odysseus/` creada antes del renombrado sigue reconociéndose si no existe ya una `.faustus/`; y (2) un checkpoint existente con el marcador `ODYSSEUS_WORKSPACE` se sigue leyendo junto al nuevo `FAUSTUS_WORKSPACE`.
