...
**Verificado.** `tests/test_connectors.py`, `test_launch_profiles.py`, `test_connector_policy.py`, `test_candidature_responses.py`, `test_calendar_external_ref.py`, `test_email_list_filters.py`; `tsc`, `vite build`, `i18n --check`; suite nube entera: ver PENDIENTES «Última evidencia». Queda la vista en vivo en el 7001 y la conexión real a Jobhunter desde su máquina (con su Qwen en Ollama para el listado de contexto y ofertas, sin envíos).
