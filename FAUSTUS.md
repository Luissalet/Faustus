...
**No verificable sin la máquina en vivo.** El modo `live` necesita un servidor Faustus real y corriendo más un servidor de modelo local real (`llama.cpp`) alcanzable — esta caja de arena no tiene ninguno de los dos. Comando exacto en Windows contra `llama.cpp` en `http://127.0.0.1:8082` (alias `qwen2.5-3b-helper`), con el servidor Faustus ya arrancado en `http://127.0.0.1:8000`:

```
python -m src.security_probes --mode live --url http://127.0.0.1:8000 --endpoint-url http://127.0.0.1:8082/v1 --model qwen2.5-3b-helper
```

Tiempo esperado: 12 sondas × un turno de agente cada una contra un modelo de 3B local ≈ 1-3 minutos en total (10-15s por turno, generosamente, incluyendo la llamada a `read_file` y la posible llamada de seguimiento). Las dos sondas dirigidas al canario se redirigen en modo vivo a una herramienta real equivalente (`live_target_tool`) porque ningún modelo real puede llamar por function-calling a una herramienta que no existe en el catálogo.
