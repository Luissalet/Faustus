...
- **Benchmark, migración, distribución y superficie (A32-A36).** `scripts/benchmark_matrix.py` nunca reporta coste `0` cuando algún intento no fue priceable — reporta `"unknown"` — y una celda sin dato cuenta como `MISSING` en el denominador, no se descuenta. `src/migration/` importa el formato de intercambio documentado en `MIGRACION.md` sin escribir nunca el valor de una credencial a `DATA_DIR`. `docs/distribution/LIFECYCLE.md` + `scripts/lifecycle_check.py` verifican backup→restore bit a bit y que desinstalar nunca borra `data/` sin `--purge`. `SUPERFICIE_PRODUCTO.md` mapea edit/retry/branch/export de una sesión a su ruta real.
...
**Ficheros.** `docs/spec/paridad/{ESTADO_ACEPTACION,MATRIZ_PARIDAD,SCHEDULER_POLITICA,MIGRACION,SUPERFICIE_PRODUCTO}.md`, `docs/distribution/LIFECYCLE.md`, `src/{tool_discovery,loop_breaker,budget_account,tool_result_offload,run_model_pin,service_identity,sandbox_provider,mcp_tool_cache,skill_sources}.py`, `src/code_mode/`, `src/harness_evolution/`, `src/migration/`, `core/oidc.py`, `studio/embed/`, `sdk/ts/`, `scripts/{benchmark_matrix,lifecycle_check}.py`, `tests/acceptance/test_a*.py`.
