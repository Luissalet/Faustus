...
function Preview(){
 ...
 <Composer draft={draft} setDraft={setDraft} busy={false} pending={false} knobs={knobs} setKnobs={setKnobs} workspace="" onPickWorkspace={()=>{}} onClearWorkspace={()=>{}} gen={{}} onClearGen={()=>{}} attachments={attachments} setAttachments={setAttachments} sessionId={session} onSend={text=>{setSent(JSON.stringify({text:withImageReferences(text,attachments),attachments:attachments.map(a=>a.name)}));setDraft('');setAttachments([]);}} onStop={()=>{}} onVoice={()=>{}} onNotice={setNotice} modelPicker={<button type="button" className="fs-studio__chip fs-studio__chip--model"><span>qwen3.8:27b-preview</span></button>} behaviorModes={[]} behaviorModeId={null} onPickBehaviorMode={()=>{}} textareaRef={textarea}/>
