...
import { t } from '../i18n';
import { responseReason } from '../adapters/api';
import './settings.css';
...
export default function ModelConnections({open, onOpenChange, onDone}: {
  ...
  const [kind, setKind] = useState<'api' | 'claude' | 'codex'>('api');
  ...
  const connect = async () => {
    ...
    try {
      const response = await fetch(`/api/agent-runners/${kind}/model`, {
      ...
      if (!response.ok) {
        setMessage(await responseReason(response, `/api/agent-runners/${kind}/model`, response.status === 409 ? t('Enable external agent runners in Settings before connecting a client.')
  ...
  return <Dialog open={open} onOpenChange={next => {if (!busy) onOpenChange(next);}}
    ...
    <div className="fs-set__row-actions" role="group" aria-label={t('Connection type')}>
      ...
      <Button label="Codex" aria-pressed={kind === 'codex'} disabled={busy} variant={kind === 'codex' ? 'primary' : 'secondary'} onClick={() => {setKind('codex'); setMessage('');}} />
    </div>
    {kind === 'api' ? <ProviderConnect embedded onDone={onDone} /> : <form className="fs-set__provider-form" onSubmit={e => {e.preventDefault(); void connect();}}>
      <p className="fs-set__help">{kind === 'codex'
        ? t('Use the official Codex sign-in on the Faustus server. Text only; Faustus runs the tools. Subscription limits apply. No automatic switch to paid API.')
        : t('Use the official Claude Code sign-in on the Faustus server. Text only; Faustus runs the tools. Subscription limits apply. No automatic switch to paid API.')}</p>
      ...
      <a href={kind === 'codex' ? 'https://learn.chatgpt.com/docs/auth' : 'https://code.claude.com/docs/en/authentication'} target="_blank" rel="noopener noreferrer">{t('Setup and sign-in help')}</a>
      <Button label={busy ? t('Checking connection…') : kind === 'codex' ? t('Connect Codex') : t('Connect Claude Code')} variant="primary" loading={busy} onClick={() => void connect()} />
    </form>}
