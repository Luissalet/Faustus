...
export async function startDictation(lang = 'es-ES'): Promise<Dictation> {
  // Server route: record with MediaRecorder, POST the blob.
  let stream: MediaStream;
  try {
    stream = await navigator.mediaDevices.getUserMedia({ audio: true });
  } catch {
    throw new Error('No hay micrófono disponible o no has dado permiso.');
  }
...
function browserTranscribe(lang: string): Promise<string> {
  ...
  if (!Ctor) return Promise.reject(new Error('Este navegador no reconoce voz y el servidor está en modo navegador.'));
  return new Promise((resolve, reject) => {
    ...
    rec.onerror = (e) => reject(new Error(e.error ?? 'reconocimiento fallido'));
...
export async function speak(text: string): Promise<() => void> {
  stopSpeaking();
  const clean = text.replace(/```[\s\S]*?```/g, ' código ').replace(/[*_#>`]/g, '').slice(0, 4000);
  ...
  if (!('speechSynthesis' in window)) throw new Error('No hay voz disponible: el servidor no sintetiza y el navegador tampoco.');
