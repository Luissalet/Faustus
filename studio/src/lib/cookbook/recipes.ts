...
const BRIDGE = 'BRIDGE_DIR="${FAUSTUS_ROOT:-$PWD}/swift/faustus-mlx-image-bridge"';
const BRIDGE_CHECK = `${BRIDGE}; test -d "$BRIDGE_DIR" || { echo "Run this from an Faustus checkout that includes swift/faustus-mlx-image-bridge, or set FAUSTUS_ROOT=/path/to/faustus."; exit 1; }`;
...
export const RECIPES: Recipe[] = [
  ...
  { backend: 'mlx_vlm', label: 'MLX image models (HiDream)', match: any, variants: { pip: { commands: ['python -m pip install -U fastapi uvicorn python-multipart mlx mlx-vlm "transformers>=4.57.0,<6.0" huggingface_hub safetensors numpy pillow tqdm sentencepiece hf_transfer'] } } },
  {
    ...
    match: any,
    variants: { pip: { commands: ['python -m pip install -U fastapi uvicorn python-multipart pillow huggingface_hub', BRIDGE_CHECK, `${BRIDGE}; cd "$BRIDGE_DIR" && swift build -c release --product faustus-mlx-inpaint`, `${BRIDGE}; mkdir -p "$HOME/.local/bin" && cp "$BRIDGE_DIR/.build/release/faustus-mlx-inpaint" "$HOME/.local/bin/faustus-mlx-inpaint"`, MLX_METALLIB] } },
  },
  {
    backend: 'mlx_ddcolor_swift',
    label: 'MLX image editing (DDColor)',
    match: any,
    variants: { pip: { commands: ['python -m pip install -U fastapi uvicorn python-multipart pillow huggingface_hub', BRIDGE_CHECK, `${BRIDGE}; cd "$BRIDGE_DIR" && swift build -c release --product faustus-mlx-colorize`, `${BRIDGE}; mkdir -p "$HOME/.local/bin" && cp "$BRIDGE_DIR/.build/release/faustus-mlx-colorize" "$HOME/.local/bin/faustus-mlx-colorize"`, MLX_METALLIB] } },
  },
