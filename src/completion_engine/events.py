"""completion_engine/events.py -- a run's depth decisions as a stream you may rejoin.

Copied deliberately from `src/delta_engine/events.py`, which copied
`src/state_mirror/events.py`, down to the shape of the ring buffer: same
`since(cursor)` resume, same announcement when the capacity dropped something,
same redaction through the envelope's own `Event.redact`, and the same UNNAMED
SSE frame with the event's name inside the JSON.
...
Keyed by OWNER, like the State Mirror and the Delta Engine and unlike the
council. What this stream carries is not about a conversation: the same run is
watched by the page that started it, by whatever is deciding whether to raise a
budget, and by the reader who opens the account months later. Keyed by session,
each of those would need the same fact published into every open session --
numbered differently in each, so no two consumers could agree on a cursor. One
stream per owner is the only key under which "how far this run has gone so far"
has one sequence number.
...
* **`completion_converged` and `completion_budget_exhausted` are never emitted
  for the same stop.** This is the rule this module exists to hold, and
  `src/contracts/event.py` states it beside the names themselves. "There was
  nothing left worth doing" and "we ran out" lead to OPPOSITE next actions --
  the first says the run is finished, the second says raise the budget -- and
  one event covering both would make the second unreportable, which is exactly
  how a budget that is too small stays too small forever. `stop_event()` is the
  only translation from a `STOP_REASONS` to an event name, it returns one name,
  and a test walks every stop reason to prove no reason can produce both.

* **An event carries references and counts, never the decision.** A
  `CompletionDecision` is every executed, rejected and deferred candidate with
  its evidence; put one in `completion_decision_recorded` and a greedy run
  pushes a megabyte down an SSE connection to say one word. `run_id` is on the
  event itself for the same reason `delta_id` is on a `DeltaEvent`: every
  consumer filters on it, and making them parse each frame to discard most of
  them is the cost nobody notices until there are a thousand frames.

* **No secret leaves in an event.** The payload goes through `Event.redact()`,
  the envelope's own structural redactor, not through a fifth copy of the same
  regexes. It fails CLOSED: a payload that could not be scrubbed is a payload
  that is not published. A candidate names paths and quotes a `detail` written
  about somebody's file, so this is not theoretical here.
...
* **A name this module does not declare is published as `completion_error`.**
  Never dropped, never raised. An unroutable name in a stream is a string
  nothing listens to, and a run must not die of a typo in a progress line.

THREADING. The buffer is guarded by a `threading.RLock`, so `publish()` is safe
from any thread -- which matters here because discovery and verification run on
workers while a route reads the same stream. Waiters are `asyncio.Event`s
remembered with the loop they were made in and woken through
...
into a full-length timeout that looks exactly like a run that has hung.
"""
...
from src.completion_engine.contracts import HONEST_STOPS, STOP_REASONS, new_id
...
#: The fourteen names this subsystem may publish. Underscored, matching the
#: `project_context_*`, `council_*`, `state_*` and `delta_*` blocks already in
#: `src/contracts/event.py::EVENT_NAMES` -- a dotted spelling would be a sixth
#: dialect in a tuple that already carries five.
#:
...
#: purpose. See rule 1 of the module docstring, and `stop_event()`, which is the
#: only thing allowed to choose between them.
COMPLETION_EVENTS: Tuple[str, ...] = (
    "completion_contract_created",
    "completion_scope_compiled",
    "completion_layer_opened",
    "completion_candidate_discovered",
    "completion_candidate_rejected",
    "completion_batch_started",
    "completion_layer_verified",
    "completion_layer_completed",
    "completion_scope_expansion_requested",
    "completion_frontier_recomputed",
...
#:
#: `contract_id`, `decision_id`, `scope_envelope_id`, `mode`, `layer` and
#: `candidate_id` are here rather than in each event's own body because every
#: completion event is about one run, under one contract, inside one envelope,
#: on one layer -- and a consumer that had to write `payload.get("layer", "")`
#: on every branch eventually forgets a branch. `mode` in particular is on every
#: frame because it is what makes the rest of the frame mean anything: the same
#: `completion_layer_opened` is routine in `maximalist` and a policy violation
#: in `literal`.
COMMON_PAYLOAD_KEYS: Tuple[str, ...] = (
    "owner", "project_id", "run_id", "session_id", "contract_id", "decision_id",
    "scope_envelope_id", "mode", "layer", "candidate_id", "actor_id",
    "event_id", "causation_id", "correlation_id", "at",
)

#: How many events one owner's stream keeps. A greedy run publishes one frame
#: per candidate at its noisiest, so this is several whole runs of history --
#: enough that a page reloading mid-run resumes without a hole, and bounded so
#: that a process left running for a week does not hold a week of them. Past it
#: the oldest are dropped and `since()` says so.
...
_ERROR_EVENT = "completion_error"

#: The ONE translation from a stop reason to the name that reports it, and the
#: reason it is a map rather than an `if` in each caller.
#:
#: `HONEST_STOPS` is the frozen contract's own answer to "did this run stop
#: because there was nothing left worth doing", so the honest stops -- and only
#: those -- become `completion_converged`. `budget` becomes
#: `completion_budget_exhausted`. Nothing else can produce either name, and
#: because `budget` is not in `HONEST_STOPS` (which `CompletionDecision.parse`
#: also relies on when it refuses a `converged` beside an exhausted line), no
#: stop reason can produce both. Every other stop is an INTERRUPTION and is
#: reported as one: `completion_decision_recorded` says the account was written
#: without claiming either of the two sentences that lead somewhere.
_STOP_EVENTS: Dict[str, str] = dict(
    [(reason, "completion_converged") for reason in HONEST_STOPS]
    + [("budget", "completion_budget_exhausted")]
)


def stop_event(stop_reason: str) -> str:
    """The event name that reports this stop. Never both of the two.

    A stop reason this build does not know answers `completion_error` rather
    than guessing: the two names this function chooses between lead to opposite
    next actions, and picking one of them for a word nobody declared would be
    the guess with the worst possible payoff -- an unrecognised stop reported as
    convergence is a run nobody ever raises the budget for.
    """
    name = str(stop_reason or "")
    if name not in STOP_REASONS:
        logger.warning("completion events: %r is not a stop reason; reported as "
                       "%s rather than guessed at", stop_reason, _ERROR_EVENT)
        return _ERROR_EVENT
    return _STOP_EVENTS.get(name, "completion_decision_recorded")
...
def _redact(payload: Mapping[str, Any]) -> Tuple[Dict[str, Any], int]:
    ...
    blanking secret-looking keys and counting what it blanked. Writing a sixth
    ...
    payload we do not publish. A completion payload quotes a candidate's
    `detail` and names the paths it would touch, so the thing being protected
    here is real.
    """
    try:
        from src.contracts.event import Event

        scrubbed = Event(name="completion", data=dict(payload)).redact(())
        return dict(scrubbed.data), int(scrubbed.redactions)
    except Exception as exc:  # noqa: BLE001 - never raise on a publish path
        logger.warning("completion events: redaction unavailable, payload "
                       "dropped: %s", exc)
...
class CompletionEvent:
    """One thing the engine decided, numbered so a client can resume after it.
    ...
    `run_id` is lifted out of the payload and onto the event because every
    consumer of this stream filters on it: a page watching one run wants that
    run's frames, and making it parse every frame to discard most of them is the
    cost that appears only under load.
    """
    ...
    def sse(self) -> str:
        ...
        own frame and never the whole stream -- and a completion payload carries
        whatever an estimator put in a score or a scope narrowing put in a
        reason, which is exactly where an unserialisable value comes from.
        """
        try:
            body = json.dumps(self.to_dict(), ensure_ascii=False,
                              sort_keys=True, default=str)
        except Exception as exc:  # noqa: BLE001 - one bad frame, not a dead stream
            logger.warning("completion events: %s (%s) would not serialise: %s",
...
class CompletionEventStream:
    """One owner's ordered, resumable, redacted log of how far a run went.

    In memory and bounded. The durable half lives in `persistence.py` -- the
    decision is the account and this is the live commentary. The two are
    ...
    def __init__(self, owner: str, *, capacity: int = DEFAULT_CAPACITY) -> None:
        ...
        self._events: Deque[CompletionEvent] = deque(maxlen=self.capacity)
    ...
    def publish(self, name: str, **payload: Any) -> CompletionEvent:
        """Number, redact, store and broadcast one event. Never raises.

        An unknown name becomes `completion_error` carrying `unknown_event`: the
        ...
        whoever has to find the typo. A run that died because one publisher
        misspelled a progress line would be a run that reports nothing about
        work that was fine.
        ...
        known = raw in COMPLETION_EVENTS
        body = dict(payload)
        if not known:
            logger.warning(
                "completion events %r: %r is not a completion event name; "
                "published as %s so that nothing routes on a name this module "
                "never declared", self.owner, raw, _ERROR_EVENT)
        ...
        if late:
            logger.info("completion events %r: %s arrived after close and is "
                        "kept, marked late, and acted on by nothing",
                        self.owner, raw)
            return event
        self._wake()
        return event

    def publish_stop(self, stop_reason: str, **payload: Any) -> CompletionEvent:
        """Publish the ONE event that reports this stop. Rule 1, as a method.

        Here so that no caller has to hold the rule in its head: `stop_event()`
        chooses the name, this passes the reason along in the payload, and there
        is no path through which a caller can emit `completion_converged` and
        `completion_budget_exhausted` for one stop except by calling `publish`
        twice on purpose.
        """
        body = dict(payload)
        body.setdefault("stop_reason", str(stop_reason or ""))
        return self.publish(stop_event(stop_reason), **body)

    def _make(self, seq: int, name: str, data: Dict[str, Any],
              redactions: int, late: bool) -> CompletionEvent:
        event_id = new_id("completion_event")
        created_at = now_iso()
    ...
    def since(self, cursor: int = 0, *,
              limit: int = DEFAULT_LIMIT) -> Tuple[List[CompletionEvent], int, bool]:
        ...
        refresh, and the hole costs a rejected candidate nobody knows was
        refused, which on this stream is the same as a budget nobody knows to
        raise. It is a flag rather than a synthetic event because a marker in
        the list would have to be given a `seq` and a name, and every consumer
        would then have to know which frames are real.
        """
        try:
            after = int(cursor)
        except (TypeError, ValueError):
            logger.debug("completion events %r: unreadable cursor %r, treated "
                         "as 0", self.owner, cursor)
            after = 0
        if after < 0:
            after = 0
        try:
        ...
        if gap:
            logger.info("completion events %r: consumer resumed at %d but the "
                        "buffer starts at %d; %d event(s) are gone", self.owner,
                        after, buffered[0].seq, (buffered[0].seq - 1) - after)
    ...
    def _wake(self) -> None:
        ...
        full-length timeout that looks exactly like a run that has hung.
        Discovery and verification run on worker threads, so this path is the
        ...
        for waiter, loop in waiters:
            try:
                ...
                logger.debug("completion events %r: waking a waiter failed: %s",
                             self.owner, exc)
    ...
    def close(self) -> None:
        ...
        with self._guard:
            if self._closed:
                return
            self._closed = True
        logger.debug("completion events %r: closed at seq %d", self.owner,
                     self.last_seq())
        self._wake()
...
_STREAMS: Dict[str, CompletionEventStream] = {}
_REGISTRY_GUARD = threading.Lock()


def stream_for(owner: str) -> CompletionEventStream:
    """The stream for this owner, made once and kept."""
    key = str(owner or "")
    with _REGISTRY_GUARD:
        stream: Optional[CompletionEventStream] = _STREAMS.get(key)
        if stream is None:
            stream = _STREAMS[key] = CompletionEventStream(key)
            logger.debug("completion events %r: stream created", key)
        return stream
...
def reset_streams() -> None:
    ...
    for stream in streams:
        try:
            ...
            logger.debug("completion events: closing %r failed: %s",
                         stream.owner, exc)
