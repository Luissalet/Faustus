...
BUILTIN_TOOL_DESCRIPTIONS: Dict[str, str] = {
    ...
    "code_graph_impact": "From one symbol, or from every symbol the current git diff touches, BFS over incoming callers up to a depth to answer what else can break and which tests to run -- each reached symbol with file:line, depth and certainty, the affected test files/functions, and a ready pytest command. Use for 'what depends on this', 'what would break if I change X', 'qué se rompe si toco esta función', 'qué tests debería correr después de este cambio'.",
