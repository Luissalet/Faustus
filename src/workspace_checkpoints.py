...
def _git_env() -> Dict[str, str]:
    ...
    env.update({
        ...
        "GIT_AUTHOR_EMAIL": "faustus@localhost",
        "GIT_COMMITTER_NAME": "Faustus checkpoints",
        "GIT_COMMITTER_EMAIL": "faustus@localhost",
...
def _write_exclude(root: str, gd: str, max_file_mb: float) -> None:
    ...
    if now - _EXCLUDE_CACHE.get(gd, 0.0) < _EXCLUDE_TTL_S:
        return
    lines = ["# generated by Faustus — edit FAUSTUS settings, not this file"]
...
def _ensure_repo(root: str) -> bool:
    ...
    if not os.path.isdir(os.path.join(gd, "objects")):
        ...
        try:
            ...
            with open(os.path.join(gd, "FAUSTUS_WORKSPACE"), "w", encoding="utf-8") as f:
                f.write(root + "\n")
        except OSError:
            pass
