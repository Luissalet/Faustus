...
# Module-level so tests can point the stderr logs somewhere disposable (the
# pattern src/command_guard.py already uses).
...
def _format_mcp_connection_error(name: str, command: str = "", args: Optional[List[str]] = None,
                                 error: Exception = None, stderr_tail: str = "") -> str:
    """Return a user-actionable MCP connection error message.

    `stderr_tail` is the last few lines the server printed before it died
    (FAUSTUS). Without it the packaged Windows build has no console, so a server
    that cannot start prints `node: not found` into the void and the UI shows a
    generic transport error — the app's worst debugging experience, and the
    reason is sitting right there in the child's stderr.
    """
    args = args or []
...
# ── Per-server stderr capture (FAUSTUS) ───────────────────────────────────
#
# `stdio_client(server_params)` with no `errlog` lets the SDK default it to
# `sys.stderr`. In the packaged Windows build there is no console attached, so
# everything the child says is discarded. Each server gets its own file instead.

# Bytes kept per server. Enough to hold a Node stack trace and the lines around
# it; small enough that a server in a crash loop cannot fill the disk.
MCP_STDERR_MAX_BYTES = 256 * 1024
# Lines of that file quoted back in a failed connect's error message.
MCP_STDERR_TAIL_LINES = 12
MCP_STDERR_TAIL_CHARS = 2000

_SAFE_SERVER_ID_RE = re.compile(r"[^A-Za-z0-9._-]+")


def mcp_log_dir() -> str:
    return os.path.join(DATA_DIR, "mcp_logs")


def stderr_log_path(server_id: str) -> str:
    """`DATA_DIR/mcp_logs/<server>.stderr.log`.

    The id is sanitised because it reaches a filename: ids are generated by the
    app today, but `..`/separators in one would put a log outside DATA_DIR.
    """
    safe = _SAFE_SERVER_ID_RE.sub("_", str(server_id or "server")).strip("._-") or "server"
    return os.path.join(mcp_log_dir(), f"{safe[:80]}.stderr.log")


def _trim_stderr_log(path: str, max_bytes: Optional[int] = None) -> None:
    """Cap the log, dropping from the FRONT at a line boundary.

    Truncating from the end would throw away the crash and keep the boot banner,
    which is exactly backwards: the useful line is always the last one.

    `max_bytes` defaults to the module constant read AT CALL TIME, not bound
    into the signature: a default argument evaluated at import makes the cap
    look configurable while ignoring anything that changes it.
    """
    if max_bytes is None:
        max_bytes = MCP_STDERR_MAX_BYTES
    try:
        ...
        logger.debug("could not trim MCP stderr log %s: %s", path, exc)


def open_stderr_log(server_id: str) -> Optional[TextIO]:
    """An append handle for this server's stderr log, or None. Never raises.

    A log that cannot be opened must not stop a server from connecting — the
    capture is a debugging aid, not a precondition — so every failure returns
    None and `stdio_client` keeps the SDK default.
    """
    ...
    try:
        ...
        try:
            # The logs sit inside DATA_DIR and hold whatever a third-party
            # server chose to print, which may include a token it echoed back.
            # Nothing the app serves should be able to read them.
            os.chmod(directory, stat.S_IRWXU)
        except OSError:
            pass
        _trim_stderr_log(path)
        ...
        logger.debug("could not open MCP stderr log for %s: %s", server_id, exc)
        return None
    except Exception as exc:  # noqa: BLE001 - a log is never worth a connection
        logger.debug("could not open MCP stderr log for %s: %s", server_id, exc)
        return None


def read_stderr_tail(server_id: str, lines: int = MCP_STDERR_TAIL_LINES) -> str:
    """The last few lines this server printed, for the error the UI shows."""
    path = stderr_log_path(server_id)
    try:
        with open(path, "rb") as fh:
            try:
                fh.seek(-MCP_STDERR_MAX_BYTES, os.SEEK_END)
            except OSError:
                fh.seek(0)
            raw = fh.read()
    except OSError:
    ...
    if not text:
        return ""
    tail = "\n".join(text.split("\n")[-max(1, int(lines)):]).strip()
    if len(tail) > MCP_STDERR_TAIL_CHARS:
        tail = "…" + tail[-MCP_STDERR_TAIL_CHARS:]
    return tail


# ── Child environment (FAUSTUS) ───────────────────────────────────────────
#
# `env={**os.environ, **env}` handed a third-party MCP subprocess every provider
# API key in the process and the internal loopback token — the one 26.5 found is
# privileged enough to reach /api/storage/*. A server needs a PATH to be found,
# a HOME to write its cache, and its own declared variables. It does not need
# the keys to somebody else's model provider.
#
# The base set is the SDK's own DEFAULT_INHERITED_ENV_VARS plus what a Windows
...
def server_inherits_env(srv: Any) -> bool:
    """Read a DB row's `inherit_env`, defaulting to True.

    True is the compatible answer for every shape the value can arrive in: the
    column missing (an old DB whose migration has not run), the value NULL (a
    row written before the column existed), or an object that does not carry the
    attribute at all. Nothing already working may lose its environment because a
    read went sideways.
    """
    try:
        value = getattr(srv, "inherit_env", None)
...
def new_server_inherits_env() -> bool:
    ...
    try:
        from src.settings import get_setting
        return not bool(get_setting("agent_mcp_min_env", True))
    except Exception:  # noqa: BLE001 - settings backend unavailable
        return False


def build_server_env(declared: Optional[Dict[str, str]], inherit_env: bool) -> Optional[Dict[str, str]]:
    """The env handed to `StdioServerParameters`.

    `inherit_env=True` is what every server configured before this change keeps,
    byte for byte: the full process environment with the server's own values on
    top, and `None` (which makes the SDK build its own safe default) when the
    server declares nothing. `False` is the minimal set.
    """
...
class McpManager:
    ...
    async def connect_server(
        ...
        `inherit_env` (FAUSTUS) decides what the child process gets. `None` means
        "the caller did not say", which is TODAY's behaviour — the full process
        ...
        try:
            if transport == "stdio":
                res = await self._connect_stdio(
                    ...
                    inherit_env=True if inherit_env is None else bool(inherit_env),
                )
            elif transport == "sse":
    ...
    async def _connect_stdio(self, server_id: str, name: str, command: str, args: List[str],
                             env: Dict[str, str], inherit_env: bool = True) -> bool:
        ...
        `inherit_env` defaults to True — today's behaviour — so a caller that
        does not know about it cannot accidentally starve a working server.
        The child's stderr goes to `DATA_DIR/mcp_logs/<server>.stderr.log`
        instead of a console that does not exist in the packaged build.
        """
        try:
        ...
        server_params = StdioServerParameters(
            command=command,
            args=args,
            env=build_server_env(env, inherit_env),
        )

        errlog = open_stderr_log(server_id)
        if errlog is not None:
            try:
                import datetime as _dt
                errlog.write(f"\n=== {_dt.datetime.now().isoformat(timespec='seconds')} "
                             f"starting {name} ({server_id}): "
                             f"{' '.join([command or '', *args]).strip()} "
                             f"[env: {'inherited' if inherit_env else 'minimal'}] ===\n")
                errlog.flush()
            except Exception:  # noqa: BLE001 - a header is never worth a connection
                pass

        def _close_errlog() -> None:
            if errlog is None:
                return
            try:
                errlog.flush()
        ...
        async def _owner():
            ...
            try:
                ...
                # Only after the child is gone: the fd is duped into it, and the
                # lines we want in the error message are the ones it wrote last.
                _close_errlog()
                if not ready.done():
        ...
        self._connections[server_id] = {
            ...
            "identity": identity,
            # Which environment this child actually got, and where its output
