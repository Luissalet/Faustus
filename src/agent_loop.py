...
TOOL_SECTIONS = {
    ...
    "whatsapp_send": "- ```whatsapp_send``` - Send a WhatsApp text from the user's own account: {\"to\":\"Ana\",\"text\":\"…\"}. `to` = contact name as saved, phone with country code, or jid; on an ambiguous name the tool lists candidates — ask which. Write exactly what the user asked to say, in their voice; the user approves the send.",
