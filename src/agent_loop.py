...
Generate an image. Line 1 = description, line 2 = model name, line 3 = WxH (e.g. 1024x1024), line 4 = quality.""",
    ...
    "manage_tasks": "- ```manage_tasks``` — Create and manage scheduled background tasks (recurring AI jobs). Args (JSON): {\"action\": \"list|create|edit|delete|pause|resume|run\", ...}. Built-in actions that need no model (task_type=action, params as JSON in prompt): weather_report {\"place\",\"when\"}, news_brief {\"topic\",\"hours\"}, watch_page {\"url\",\"mode\":\"availability\"} (report only on change), mail_digest {\"hours\"}. Example — '¿qué tiempo hace en Móstoles mañana? que se repita cada día': {\"action\":\"create\",\"name\":\"Tiempo en Móstoles\",\"task_type\":\"action\",\"action_name\":\"weather_report\",\"prompt\":\"{\\\"place\\\":\\\"Móstoles\\\",\\\"when\\\":\\\"tomorrow\\\"}\",\"schedule\":\"daily\",\"scheduled_time\":\"08:00\",\"timezone\":\"Europe/Madrid\",\"pin_to_home\":true} — then run it once (action run) so the card has content.",
