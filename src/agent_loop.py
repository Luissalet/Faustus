...
`calendar` accepts a name ("Main") or short-id prefix.""",
    ...
    "ask_user": "- ```ask_user``` — Ask the user to DECIDE when the task is ambiguous and the answer changes what you build next: which approach, which library or storage, which files or behaviour they meant, which scope (a request like \"add auth\" or \"implement X\" with several sensible designs is exactly this). Args (JSON): {\"question\": \"one clear question\", \"options\": [{\"label\": \"short choice\", \"description\": \"one line: what happens if they pick it\"}, ...], \"multi\": false?}. 2-4 options, the recommended one FIRST with \"(recommended)\" in its label; set \"multi\": true for a checklist when several may apply. The user gets one button per option, a checklist when multi, and always a free-text line to answer in their own words — so never add an \"Other\" option yourself. Calling this ENDS your turn and their answer comes back as your next message. Do not ask what has an obvious default or what you can check yourself; ask once, with all the decisions you need in that one question when they are related.",
