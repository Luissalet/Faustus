...
DATA_DIR = os.getenv("FAUSTUS_DATA_DIR", get_default_data_dir())

# Data file paths
# Single source of truth: every persisted file/dir lives under DATA_DIR, which
# is the ONLY place FAUSTUS_DATA_DIR is read. Import these constants instead of
...
MAIL_ATTACHMENTS_DIR = os.getenv("FAUSTUS_MAIL_ATTACHMENTS_DIR", os.path.join(DATA_DIR, "mail-attachments"))
...
# Ceiling for read_file / document preview. src/read_plan.py scales the per-read
# cap DOWN from here when the model's context window is known and small (20k
# characters is a third of an 8k window), and an un-ranged read of a file over
# the cap answers with a symbol index instead of a blind slice off the top. This
# number stays the maximum any single read may return.
...
def internal_api_base() -> str:
    """Base URL for in-process loopback calls to Faustus's own API.

    Agent tools and background jobs reach admin-gated routes by calling the
    running server over HTTP. Resolution order:
      1. FAUSTUS_INTERNAL_BASE  - explicit override (e.g. behind a TLS proxy).
    ...
    """
    override = os.environ.get("FAUSTUS_INTERNAL_BASE")
    if override:
