...
async def action_run_script(owner: str, script: str = "", host: str = "", **kwargs) -> Tuple[str, bool]:
    ...
    target_host = (host or os.getenv("FAUSTUS_SCRIPT_HOST", "localhost")).strip()
            ...
            try:
                ...
                if newest is not None:
                    STATE_FILE.write_text(json.dumps({
                        ...
                        "last_run_at": datetime.now(_utc_timezone.utc).replace(tzinfo=None).isoformat(),
                        "scanned": len(events),
                        "removed": len(removed),
...
async def action_email_auto_translate(owner: str, **kwargs) -> Tuple[str, bool]:
    ...
    try:
        ...
        def _store(
            ...
            *,
            ...
            try:
                ...
                """, (
                    body_hash, owner, target_language, uid, folder, subject, sender,
                    translation, 1 if same_language else 0, model_used, _dt.now(_utc_timezone.utc).replace(tzinfo=None).isoformat(),
                ))
                c.commit()
            finally:
                c.close()
        ...
        since = (_dt.now(_utc_timezone.utc).replace(tzinfo=None) - _td(days=days_back)).strftime("%d-%b-%Y")
        examined = 0
        cached = 0
        translated = 0
        same_language = 0
...
async def action_classify_events(owner: str, **kwargs) -> Tuple[str, bool]:
    ...
    try:
        ...
        db = SessionLocal()
        try:
            now = datetime.now(_utc_timezone.utc).replace(tzinfo=None)
...
async def action_learn_sender_signatures(owner: str, **kwargs) -> Tuple[str, bool]:
    ...
    try:
        ...
        cutoff_iso = (_dt.now(_utc_timezone.utc).replace(tzinfo=None) - _td(days=30)).isoformat()
        ...
        for addr, msgs in eligible[:20]:  # cost cap per run
            ...
            try:
                ...
                conn.execute(
                    ...
                    "VALUES (?, ?, ?, ?, ?, ?, ?)",
                    (addr, owner_value, cached_sig, len(bodies), _dt.now(_utc_timezone.utc).replace(tzinfo=None).isoformat(), model, "llm"),
                )
                conn.commit()
                conn.close()
                analyzed += 1
...
async def action_check_email_urgency(owner: str, **kwargs) -> Tuple[str, bool]:
    ...
    try:
        ...
        STATE_PATH.parent.mkdir(parents=True, exist_ok=True)
        AGE_CUTOFF = _dt.now(_utc_timezone.utc).replace(tzinfo=None) - _td(days=7)
        TRIAGE_VERSION = 10
        CATEGORY_TAGS = {
                ...
                try:
                    ...
                    for uid_b in uids:
                        ...
                        try:
                            ...
                            _ody_origin = (msg.get("X-Faustus-Origin") or "").strip().lower()
                            _ody_kind = (msg.get("X-Faustus-Kind") or "").strip().lower()
                            ...
                            if (_ody_origin == "faustus-ui" or _ody_kind == "reminder"
                                    or _raw_subj.startswith("reminder (faustus):")
                    ...
                    if _row:
                        ...
                        _conn.execute(
                            ...
                            (_msg_id, _owner_key, _acc_id, _uid_only, _v.get("subject", ""),
                             ...
                             _dt2.now(_utc_timezone.utc).replace(tzinfo=None).isoformat()),
                        )
                        tag_write_details.append({
                            "uid": _uid_only,
                            "subject": _v.get("subject", ""),
...
BUILTIN_ACTION_INFO = {
    **_WATCH_ACTION_INFO,
    ...
    "run_script": "Run a script locally or on FAUSTUS_SCRIPT_HOST",
