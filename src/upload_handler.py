...
class UploadHandler:
    def __init__(self, base_dir: str, upload_dir: str):
        ...
        # composer sends a whole batch at once — so this must comfortably exceed
        # one batch, or a single 6+ file attach is rejected mid-batch (issue
        # #1346: "5 work, 6 fail"). Burst abuse is separately bounded by
        # max_concurrent_uploads. Headroom for a few full batches per minute.
    ...
    def _renamed_upload_index_key(self, key: str, info: Dict[str, Any], old_owner: str, new_owner: str) -> str:
        ...
        # Fallback for rows without an explicit hash (should not happen in modern Faustus)
