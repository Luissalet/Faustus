...
API_TOKEN_RULES: Tuple[Rule, ...] = (
    ...
    *(Rule(frozenset({"POST"}), "/api/codex/cookbook/" + path, ("cookbook:launch",),
    ...
    # Read-only reviews of an owned session: what it used (tokens, cache,
    # steps, cost) and what its recent turns did. A coordinator holding a
    # dispatch token reads them for the jobs it started.
    _read("/api/session/{session_id}/usage", "sessions", "agents:dispatch",
