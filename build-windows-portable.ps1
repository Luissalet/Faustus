...
<#
  Build a portable Windows distribution for Faustus.

  Output layout:
    dist\Faustus\Faustus.exe
    dist\Faustus\static\...
    dist\Faustus\scripts\...
    dist\Faustus\mcp_servers\...
    dist\Faustus\services\hwfit\data\...
...
& $pyExe -m PyInstaller --noconfirm --clean --onedir --noconsole --icon=static/icon.ico --name Faustus @dataArgs launcher.py
if ($LASTEXITCODE -ne 0) { Fail "PyInstaller build failed." }

Write-Host ""
Write-Host "Build complete." -ForegroundColor Green
Write-Host "Portable app folder: $PSScriptRoot\dist\Faustus" -ForegroundColor Green
