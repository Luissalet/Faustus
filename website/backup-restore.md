...
Faustus keeps all of your state in the `data/` directory — the SQLite database
(`app.db`), the Fernet encryption key (`data/.app_key`), the vault, memory, RAG
indexes, personal documents, and uploads. The `scripts/faustus-backup` tool
...
# reading one back
./scripts/faustus-backup verify  backups/faustus-backup-....tar.gz.enc
./scripts/faustus-backup restore backups/faustus-backup-....tar.gz.enc --yes
```
...
./scripts/faustus-backup snapshot

# List existing snapshots (most recent first)
./scripts/faustus-backup list

# Check a tarball's integrity without extracting it
./scripts/faustus-backup verify backups/faustus-backup-20260101-120000.tar.gz

# Restore (destructive — see the warning below)
./scripts/faustus-backup restore backups/faustus-backup-20260101-120000.tar.gz --yes
```
...
./scripts/faustus-backup snapshot --out /mnt/nas/faustus-$(date +%F).tar.gz

# Full snapshot including research runs and mail attachments
./scripts/faustus-backup snapshot --include-research --include-attachments
```
...
```cron
0 3 * * *  cd /path/to/faustus && ./scripts/faustus-backup snapshot --out "/mnt/nas/faustus-$(date +\%F).tar.gz"
```
...
> **not** under `./data`. `faustus-backup` therefore does not capture the Docker
