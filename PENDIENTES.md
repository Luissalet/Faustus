...
- **Arranque lento del 7006** (§199/§209/§211/§212): el 26-09 a las 16:50 tardó 2,5 min en arrancar. La pila señaló la recuperación del registro de 53 MB del examen 30, cortado a mitad. Desde `8d3dbd4d` la recuperación descarta el razonamiento y los latidos antes de parsear. En el próximo reinicio del 7006, medir el tiempo entre «Secret file hardening» y «Application startup complete».
...
- **Vigilante de avance en un turno real** (26-09, §212): con el 8081 libre, una tarea que lea mucho antes de escribir (o la batería con `agent_no_progress_rounds=6`) y comprobar en el log `rounds without progress — nudge` y que el turno escribe algo en las rondas siguientes. El 7000 ya lo tiene desplegado; en el 7006 entra con el próximo reinicio (después del examen 31).
- **Versiones por respuesta con un regenerar de verdad** (26-09, §212): regenerar una respuesta en el 7000 con el 8081 libre y comprobar ‹ 1/2 › y *Usar esta versión* sin fixture (la prueba en vivo usó una versión escrita a mano en una bifurcación).
...
- **Nota de `vision_write_every` en el examen 31** (26-09, §211): el 30 corrió con el código viejo del 7006 y acabó sin entregable tras 4 h 35 min (parado a las 16:50). En el 31, comprobar en la traza (`iq.py`) que la sexta pregunta sobre una misma imagen trae la nota y a qué hora aparece `RESPUESTA.md`.
...
- **Borrador primero en una tarea larga** (26-09): en el examen 31, comprobar que a la cuarta parte del turno el 27B crea `RESPUESTA.md` con la estructura y lo va completando.
- **Relevo antes del tope de tiempo** (26-09, §209): en la próxima tarea larga (examen 31), comprobar que al 85 % del tope el 27B deja el plan con resultados y un fichero de notas, y que el turno siguiente arranca desde ahí en vez de releerlo todo.
