...
- **`context_recall` detrás de la puerta de contexto externo**: está clasificada `read_private` (como `read_overflow`/`read_artifact`), y el propio paquete llega por el carril no confiable, que arma la puerta. En modo «ask» la primera llamada a `context_recall` de casi cualquier turno pedirá tarjeta. Decidir si se reclasifica (p. ej. como `plugins_list`) o se exime cuando el id viene del paquete de este mismo turno.
...
- **Memoria personal duplicada en modo agente**: con el motor encendido, `chat_processor.build_context_preface` sigue metiendo «saved memory» (memory.json) y el paquete puede traer los mismos `pmem:`. Sólo se reemplaza el bloque de memoria aprendida. Falta el mismo patrón de reserva para la memoria guardada (y para los documentos RAG).
- **Chat simple sin motor**: el modo chat llama a `stream_llm_with_fallback` directamente con el prefacio de `chat_processor`; nunca pasa por `deliver_round`. Para llevarlo: `build_request(agent_mode=False, consumer="chat")` + `deliver_round` antes del bucle de candidatos en `routes/chat_routes.py` (rama `chat_mode == "chat"`), insertar con la misma regla que `_insert_before_latest_user`, dejar el prefacio de memoria en reserva igual que en agente, y `observe_receipt` al guardar la respuesta.
