...
## Batería de sondas de inyección (FAUSTUS.md §148) — pendiente de verificar en la máquina en vivo

- **Modo `live` nunca se ha ejecutado de verdad**: esta caja de arena no tiene un servidor Faustus real ni `llama.cpp` alcanzables. El comando exacto en Windows contra `http://127.0.0.1:8082` (alias `qwen2.5-3b-helper`) está en FAUSTUS.md §148 — falta correrlo y anotar los resultados reales del modo vivo (¿el modelo intenta alguna de las 12 sondas por sí mismo? ¿la puerta real las bloquea todas, o alguna se comporta distinto contra un modelo real en vez del "siempre comprometido" guionizado?).
