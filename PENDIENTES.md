...
- ABIERTO (importante): **no se puede crear la primera conversación desde la pantalla de bienvenida**. Reproducción exacta: abrir `/studio` (sin `?s=`), escribir en el compositor, pulsar Enviar — el texto desaparece, no aparece conversación nueva y en la pestaña de red **no sale ni `POST /api/session`**: no llega a intentarlo. Enviar en una conversación **ya existente** funciona a la primera, siempre. Comprobado con y sin autenticación válida (ocurre igual en la instancia dev del 7001, que no exige login), así que **no es el 401**. Acotado al camino de creación de sesión. Sospecha: el compositor se remonta mientras se escribe — en esa pantalla siguen llegando `/api/projects`, `/api/tools`, `/api/chat/activity` — y el borrador local se pierde; el efecto de adopción no lo restaura porque el prop del padre está vacío. No se ha parcheado: hay que instrumentar el remontaje antes de tocarlo. Lo descartado hasta ahora, para que nadie lo repita: no es el 401 (pasa igual sin login en el 7001); no es el guard `sendingMessage.current`, cuyo `finally` sí lo limpia; y **no llega a `createSession`** — con la pestaña de red abierta no sale ninguna petición a `/api/session` al pulsar Enviar, así que el turno muere en `send()` o antes, no en la creación de sesión. Queda una guarda candidata: `if ((!message && attachments.length === 0) || sendingMessage.current) return;` — es decir, el texto llega vacío a `send`, lo que apunta otra vez al borrador local del compositor. Siguiente paso: un `console.log` en `send()` **leyendo la consola del navegador desde una herramienta que sí capture esa pestaña** (la de esta sesión no lo hizo), o un test de componente que monte Studio sin sesión, escriba y envíe.
