...
- **Hecho por separado (18-09 noche, FAUSTUS.md §113):** la carpeta de convención por proyecto (`.odysseus/` → `.faustus/`) no tenía este problema — es solo texto en el workspace del usuario, nunca en `data/` — así que se reintrodujo sola, con `src/project_conventions.py: convention_dir()` centralizando la resolución y compatibilidad de solo lectura hacia `.odysseus/`. El resto (env vars, cookie, `localStorage`, scripts) sigue pendiente del plan de migración de arriba.
