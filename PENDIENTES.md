...
- **Instalar `pypdfium2`/`pdf2image`** en un entorno con red (24-09, §181): hoy el renderizado de página de PDF sólo se probó por su camino de degradación.
...
- **Dos servidores MCP «Hoard» con WinError2** desde el 17-09 (18-09, §112): la ruta del comando ya no existe; corregirla en la configuración de conectores.
...
- **Terminal de Windows huérfano** de un arranque anterior al arreglo de `CREATE_NO_WINDOW` (17-09, §101): cerrarlo a mano y comprobar que el próximo Start no deja ninguno.
...
- **`max_duration_s` con datos reales de Windows** (FAUSTUS §149): correr `python -m src.trajectory_gate --recent 20 --spec ...` contra ejecuciones reales largas y comparar la duración de reloj con lo que se recuerda haber esperado.
...
- **`code_graph_drift()` en un repo grande real** (FAUSTUS §180): los tests usan repos sintéticos de pocos ficheros; falta ver cuánto tarda en un repo con miles de ficheros como el propio Faustus, y si el presupuesto por defecto (8-10 s) es realista.
