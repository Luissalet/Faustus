...
- **Pre-pase de análisis estático en `bug_hunt`** (26-09, resto de la auditoría de seguridad): antes de pedir casos al modelo, pasar las comprobaciones de `static_checks` sobre los ficheros objetivo, como ya hace `/review`.
- **Tope de coste por turno con modelos de pago** (26-09, auditoría de seguridad): hoy los turnos tienen tope de rondas, de tiempo y de subagentes, pero el de dinero sólo existe en los objetivos (`max_cost_usd`). Un tope por defecto por turno, como `agent_turn_max_seconds`.
