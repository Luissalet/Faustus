...
**Hecho.** WP00 (inventario contra HEAD real, sin descargar modelos), WP01 (QA01: fingerprint de receta con versión explícita), WP02 (dominio de documentos Creator con CAS+revisiones), WP03 (biblioteca/linaje sobre `artifact_catalog` existente, 28 tests), WP04 (ingesta acotada de ficheros/URLs con sniff de tipo real), WP05 (shell `/creator` en el Studio, solo presentación), WP06 (`ModelSpec` vs `DeploymentManifest`), WP07 (14 ejes de capacidad con estado known/announced/unknown), WP09 (preflight puro), WP10 (puerto de adaptadores sobre `MediaRun`), WP12 (operaciones no destructivas: `Rational`, `TimeRange`, `canvas_ops`, 23 tests), WP27 (Goal con seis tipos de criterio tipado), WP30 (inventario físico sobre `memory_budget` de INF-05), WP32 (lifecycle seguro de plugins de 4 tipos). Integración C2 destapó y corrigió un bug real de VRAM (reserva propia compitiendo consigo misma tras un reload — FAUSTUS.md §94).

**Siguiente.** WP08, WP11, WP13-16, WP20, WP24, WP36 y el resto del plan de 43. WP02/WP09/WP30 implementados y probados pero sin montar en `app.py` (fuera de la propiedad de ficheros de sus lotes — diff exacto en sus `*_wiring.md`).
