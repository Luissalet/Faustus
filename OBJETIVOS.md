...
- **Detector de «sin progreso» por huella de estado** (26-09, investigación ronda 8): `vision_write_every` corta la inspección de más sobre una imagen, pero no un bucle que alterne fuentes. La idea es una huella por ronda de lo que el turno ha fijado (ficheros escritos y su tamaño, pasos del plan cerrados, conclusiones nuevas en las notas). Si no cambia en N rondas con herramientas de lectura, se pide escribir o preguntar. Medirlo antes con las trazas de los exámenes 29 y 30.
- **Slot fijo para los ayudantes en llama-server** (26-09, investigación ronda 9): los pases internos cortos (títulos, revisiones) entran por LRU y pueden desalojar el slot de un turno largo. Si la línea `[engine] round N` muestra rondas con la caché perdida tras un pase auxiliar, mandar esos pases a un slot fijo (`id_slot`) en motores con 4 slots.
...
- **Preset MCP de documentación por repositorio** (26-09, revisión de repos): un servidor remoto de documentación de cualquier repositorio de GitHub como conector opcional de un clic, para que el agente lea la documentación de una dependencia sin buscarla en la web.
- **Auditoría de seguridad de LLM sobre Faustus mismo** (26-09, revisión de repos): pasar una lista de comprobación de seguridad de aplicaciones con LLM (inyección por herramientas, exfiltración por URLs, permisos de MCP) contra el propio código y un pre-pase de análisis estático en `bug_hunt`.
- **Revisión de código por etapas** (26-09, revisión de repos): antes de la revisión con modelo, cuatro pasos deterministas (diff, lint, tipos, tests afectados) que acoten qué mira el revisor; más barato y con menos ruido.
- **Diagnóstico de transcripciones como skill** (26-09, revisión de repos): una skill que lea la traza de un turno fallido (`llm_traces`, eventos) y diga en qué ronda y por qué se torció, para que el agente aprenda de sus propios fallos.
