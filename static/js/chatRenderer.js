    ...
    ctxRing.addEventListener('click', (e) => {
      ...
      if (compactBtn) {
        compactBtn.addEventListener('click', async (e) => {
          ...
          compactRole.textContent = 'Faustus';
...
export function addMessage(role, content, modelName, metadata) {
  try {
      ...
      for (let roundNum = firstRound; roundNum <= maxRound; roundNum++) {
        ...
        if (roundTools.length > 0) {
          ...
          for (const ev of roundTools) {
            ...
            node.innerHTML = `<div class="agent-thread-dot"></div><div class="agent-thread-header"><span class="agent-thread-icon">${statusIcon}</span><span class="agent-thread-tool">${esc(ev.tool)}</span><span class="agent-thread-status">${statusText}</span>${runBadge(ev)}<span class="agent-thread-chevron">\u25B6</span></div><div class="agent-thread-content">${evCmdHtml}${saHtml}${outHtml}${evDiffHtml}</div>`;
    ...
    var _roleText = role === 'user' ? 'You' : (isSlash || isCompacted) ? 'Faustus' : modelRouteLabel(
